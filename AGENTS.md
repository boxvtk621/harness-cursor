...
- Preserve schema/fixture compatibility, SQLite and receipt semantics, server TLS/fencing, exact versions, non-root execution, and read-only image operation. The private API has no inbound caller authentication. Architecture, API/data contract, shared infrastructure, or deploy changes require explicit approval.
